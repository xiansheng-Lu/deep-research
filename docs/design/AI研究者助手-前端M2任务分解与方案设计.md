...
> 版本: v1.0 · 起草日期: 2026-09-12 · 评审通过: 2026-09-12 · 适用范围: 前端 M2（能力补齐与工程化）
...
2. **M2 后端交付批次**（《后端详细设计》§15.2）：M2-1 意图路由 → M2-4 看板数据接口 → M2-5 用户介入 → M2-3 实时成本 → M2-2 批判收敛/分歧 API → M2-6/7 结构化报告与信源元数据 → M2-9 OpenAPI 导出。前端工作包按此顺序安排联调对接点。
...
4. WS 鉴权 M2 计划由 query token 切换为 `Sec-WebSocket-Protocol: bearer.jwt.v1` 子协议（契约草案 §5.3）；前端传输层做成可配置，**与后端同一切换，不保留双兜底**。
...
3. 指挥舱升级为实时看板：子问题进度、证据流（含 M2-7 信源元数据展示）、冲突红点提示、成本卡（70%/90% 阈值，≤3s）。
...
- 远端 CI 全程保持绿灯；M2 结束时双方对照 SDP M2 六条验收准则 + A2/A3/A8/A11/A13/A14 共同回归。
...
- 真实联调阻塞点：WP-11/12 等 M2-1 冻结；WP-14 等 M2-3/4；WP-15 等 M2-5；WP-16 等 M2-6/7。阻塞期间全部在 mock 上推进，不停工。
...
| R7 | 结构化报告强约束依赖后端真实产出 blocks | A2/A3 无法真实验收 | WP-16 先用样例 fixture 开发；联调以 M2-6 冻结 schema 为准做字段核对，markdown 渲染仅保留在生成中预览 |
...
2. 看板实时显示当前阶段、子问题 m/n、已采集证据源数量与元数据、token 用量（事件到渲染 ≤3s）；70%/90% 成本预警可见。
