...
> 版本：v0.1（草案，待后端评审）· 日期：2026-09-09

> **评审状态**：本文件为**提案**，未经后端实现评审，不作为实现契约。评审通过后，各章节按 §17 的 OpenAPI 落地方式合入[后端详细设计](./AI研究者助手-后端详细设计.md)与 OpenAPI 规范。
...
2. **契约落载体**：各章请求/响应/错误码并入后端详细设计与 OpenAPI（openapi.yaml）作为权威 schema；事件 schema 并入后端详设事件表。
3. **前端生成管线**：后端发布 openapi.json → 前端按 §3.3 `openapi-generator typescript-fetch` 生成 client 与类型；同步**删除前端 §18.1 手写草案注释与手写 d.ts 映射**，避免双源漂移。
