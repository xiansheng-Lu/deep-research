...
**文档版本**：v1.1
**创建日期**：2026-09-08
**状态**：已基线化（M1/M2 联调验收基线；2026-09-13 由"待评审"更新）

> 说明：本文描述"做什么"，不描述"怎么实现"（不指定数据库、编程语言、具体技术框架）。技术选型交由架构设计与开发计划（SDP）阶段完成。凡未定数据用 `[待确认]` 标注，凡本文推定内容用 `[假设—需确认]` 标注。
>
> **v1.1 修订（2026-09-13）**：① §4.2/§8.3/§9 报告导出 Word/PDF 由 P3/V3 勘误为 P2/V2，对齐 SDP M4-4；② §6.2 耗时指标注明里程碑硬指标以 SDP §5.1 为唯一事实源。A 系列验收编号定义未改，其里程碑映射勘误见 SDP §8.1。
...
- 企业私域连接器与血缘标注（P2）
- 报告逐句校订闭环（P2）
- 报告导出 Word/PDF（P2；2026-09-13 勘误：原列 P3，与 SDP M4-4 冲突，统一为 M4/V2 交付）
- 结构化数据资产复用（P2）
- 商业数据库接入（P3）
- 私有化部署（P3）
...
- 标准档单次研究全程尽力在可接受窗口内完成（非功能期望目标 T+30 分钟内出稿，`[待确认]`，受信源与档位影响；**里程碑硬指标以 SDP §5.1 为唯一事实源：M3 ≤ 15 分钟、M4 起 ≤ 12 分钟**，本处为非功能期望、不作为验收门槛）
...
- SDP **M4（体验打磨）/ M5（私域能力）** 对应本文档 §9 的 V2 演进（报告导出 Word/PDF、私域连接器、审计日志等；报告导出于 2026-09-13 勘误由 V3 调入 V2，对齐 SDP M4-4）
- SDP **M6（SaaS GA）** 及之后对应 V3（商业数据库、私有化部署）
...
**V2**：报告导出 Word/PDF（2026-09-13 勘误由 V3 调入，对齐 SDP M4-4）、企业私域连接器与血缘标注、报告逐句校订闭环、结构化数据资产复用、审计日志。
**V3**：商业数据库接入、面向政企/金融的私有化部署与定制模板。
