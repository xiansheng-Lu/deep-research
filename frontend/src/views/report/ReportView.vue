...
// - 含非空 blocks（真链 M2-7 终稿与 mock demo_full）→ blocks 结构化轨道；
// - blocks 为空（draft/历史报告、mock happy_path）→ marked + DOMPurify 的 markdown 轨道。
...
async function loadReport(): Promise<void> {
  ...
  try {
    ...
    // 结构化终稿增强：并行拉取终稿超集/citations/conflicts；
    // blocks 为空（draft/历史报告）或增强失败时留在 markdown 轨
  ...
  <section class="report-view">
    ...
    <template v-else>
      ...
      <template v-else-if="phase === 'ready' && run && report">
        ...
        <!-- ─── markdown 轨道（blocks 为空时：draft/历史报告、mock happy_path） ─── -->
        <template v-else>
